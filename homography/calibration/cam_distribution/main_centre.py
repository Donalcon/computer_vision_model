from math import pi
from homography.calibration.utils.data_distribution import mean_std_with_confidence_interval


def get_cam_distr(sigma_scale: float, batch_dim: int, temporal_dim: int):
    cam_distr = {
        "pan": {
            "minmax": (-pi / 3.5, pi / 3.5),  # in deg -45°, 45°
            "dimension": (
                batch_dim,
                temporal_dim,
            ),
